...
PRTRRT_COLUMNS = ["Scenario", "Cost", "Path"]
MRRT_DATA_PATH = os.path.join(PLANNING_DATA_PATH, 'MRRT_data.csv')
MRRT_COLUMNS = ["Scenario", "Cost", "Path"]
...
COMMON_COLUMNS = ["Scenario", "PRTRRT Cost", "MRRT Cost", "PRTRRT Time", "MRRT Time",
                  "PRTRRT Retry", "MRRT Retry", "PRTRRT Iter", "MRRT Iter"]
...
FINAL_GOAL1_3 = [1.1901980109241104, 0.9615559746057705, -0.5881359185350531, -1.2015471132200233,
...
FINAL_GOAL2 = [1.9414542111175392, -1.083334097524826, -1.5890062480581033, -1.8016281184439042,
                -2.857636979076863, 2.562539121147276, -0.559502583358445]
# For add obs change goal test
START_STATE3 = [-0.5578778636322044, 0.908623569993187, -0.38844591131009487, -1.2094721531006272,
...
class TestInterface():
    ...
    def init_setup(self):
        ...
        self.start_state_dict = {'change_goal': START_STATE1,
                                 ...
                                 'add_obstacle_change_goal': START_STATE3}
        self.goal_state_dict = {'change_goal': (INITIAL_GOAL1, FINAL_GOAL1_3),
                                'add_obstacle': (INITIAL_GOAL1, FINAL_GOAL2),
                                'add_obstacle_change_goal': (INITIAL_GOAL1, FINAL_GOAL1_3)}
    ...
    @abstractmethod
    def store_common_data(self, scenario, cost):
        pass
...
# Tests will be run via the test_interface_service.
    ...
    def store_data(self, scenario, goal_achieved):
        ...
        db_entry = pd.DataFrame([[scenario, cost, self.current_path]], columns=self.db_columns)
        ...
        # store common data
        self.store_common_data(scenario, cost)
        # store json data
        self.json_data['Scenarios'].append(scenario)
        self.json_data['Change States'].append(self.current_path[0])
        with open(self.json_path, 'w') as f:
            json.dump(self.json_data, f, indent=2)

    def store_common_data(self, scenario, cost):
        db_entry = pd.DataFrame([[scenario, cost, None, self.solve_time, None, self.retry_count,
                                  None, self.first_solution_iter, None]],
                                columns=self.common_db_columns)
...
# a comparison to each test stored by an PRTRRTstar run.
    ...
    def load_prtrrt_json(self):
        ...
        for scenario, state in zip(self.prtrrt_data['Scenarios'],
            ...
            response = self.run_test(scenario, self.start_state_dict[scenario])
            self.common_db_row += 1
            if response.success:
    ...
    def store_data(self, scenario, goal_achieved):
        cost = utils.path_cost(self.current_path)
        db_entry = pd.DataFrame([[scenario, cost, self.current_path]],
    ...
    def store_common_data(self, scenario, cost):
        rospy.loginfo(f"Saving common path info to...{self.common_db_path}")
        rospy.loginfo("-----------------------------------------")
        self.common_db.at[self.common_db_row, 'MRRT Cost'] = cost
        ...
        self.common_db.at[self.common_db_row, 'MRRT Iter'] = self.first_solution_iter
        self.common_db.to_csv(self.common_db_path, index=False)
