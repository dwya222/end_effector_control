...
public:
    ...
    group("panda_arm"), current_state(psm_ptr->getPlanningScene()->getCurrentStateNonConst())
  {
    ...
    joint_model_group_ = current_state.getJointModelGroup(group);
  }
  ...
  void collisionCheckLoop()
  {
    ...
    {
      ROS_INFO_STREAM("Joint '" << joint->getName() << "' satisfies bounds: " <<
                      current_state.satisfiesPositionBounds(joint, -0.5));
    }
    while (ros::ok())
    {
